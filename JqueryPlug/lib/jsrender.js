...
function $viewsDelimiters(openChars, closeChars, link) {
	...
	// Make rTag available to JsViews (or lib components) for parsing binding expressions
	$sub.rTag = "(?:" + rTag + ")";
...
function compileViewModel(name, type) {
	...
	function map(data) {
		...
		if (data) {
			...
			for (prop in data) { // Copy over any lib properties. that are not get/set properties
	...
	for (i = 0; i < l; i++) {
		...
		if ("" + node === node) {
			...
			if (tagName === "*") {
				...
				// We use views if there are props, contextual properties or args with #... (lib than #index) - but you can force
